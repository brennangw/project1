...
def report(webserviceurl):
    ...
    if request.method == 'POST':
        ...
        print now
        print session['email']
        g.db.execute('insert into public.report (reporttype, webserviceurl, reporttextblob, email, reporttime) values (%s, %s, %s, %s, %s)', [request.form['type'], request.form['url'], request.form['comment'], session['email'], now])
        print "after ex"
        g.db.commit()
        print "after co"
