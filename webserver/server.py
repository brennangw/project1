...
def webservice(webserviceurl):
    ...
    #comments
    webservice_comments = []
    cursor = g.conn.execute("SELECT * FROM public.serviceusercomment AS suc, public.serviceuser AS su WHERE suc.webserviceurl = %s AND su.email = suc.email ORDER BY suc.suctime DESC" , [webserviceurl])
    for result in cursor:
    ...
    #reports
    webservice_reports = []
    cursor = g.conn.execute("SELECT * FROM public.report AS rpt, public.serviceuser AS su WHERE rpt.webserviceurl = %s AND su.email = rpt.email ORDER BY rpt.reporttime DESC" , [webserviceurl])
    for result in cursor:
    ...
    cursor = g.conn.execute("SELECT * FROM public.representativeannouncement AS ra, public.webservicerepresentative AS su WHERE ra.webserviceurl = %s AND su.email = ra.email ORDER BY ra.ratime DESC" , [webserviceurl])
    for result in cursor:
